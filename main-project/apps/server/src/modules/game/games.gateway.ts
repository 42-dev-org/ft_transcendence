...
export class GamesGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      const isOnline = this.isUserOnline(client.user.uid);
      client.emit('user-status', {online: isOnline, offline: !isOnline, inGame: client.inGame});
    ...
    try {
      const gameCount = await this.prismaService.game.count({
        where: {
          ...
          OR: [{ status: "INVITING" }, { status: "STARTING" }],
        },
      });
      return gameCount > 0;
    } catch (error) {
  ...
  async joinInvite(
    client: Socket,
    payload: { roomId?: string; accepted: boolean }
  ) {
