        ...
        return <>
          {
            ...
            [...Array(20)].map((_, idx) => (
              <ChannelsChat onClick={() => setConversationType("channels")} time={dataChannels.time} nameChannels={dataChannels.nameChannels} msg={dataChannels.msg} pic={dataChannels.pic} key={idx} />
            ))
          }
        </>
        ...
        return <>
          {
            ...
            [...Array(20)].map((_, idx) => (
              <Userschat onClick={() => setConversationType("users")} time={data.time} name={data.name} msg={data.msg} url={data.url} key={idx} />
            ))
          }
        </>
  ...
  return (
    <Fragment>
      <ModalUI open={isAddOpen} onClose={onCloseAddModal} title='add Conversation'>
        <div className='flex justify-center items-center p-3 flex-col overflow-y-hidden max-h-72
        ' >
          ...
          <div className=' overflow-y-auto'>
            ...
            {
              [...Array(4)].map((_, idx) => (
                <ListUsersChat name={data.name} url={data.url} key={idx} className=" w-" />
              ))
            }
          </div>
      ...
      <ModalUI open={isAddOpenChannelModal} onClose={onCloseAddChannelModal} title='add Conversation'>
        <div className='flex justify-center items-center p-3 flex-col  max-h-72 gap-2' >
          <div className='flex flex-row' >
            <input type="text" className='h-7 p-1 px-3 rounded-md w-2/3 mr-2 ' />
            <select name="channels" id="" className='rounded-md' onChange={(e) => setcomponenetChannelModal(e.target.value) }>
              <option value="public">
                public
              </option>
          ...
          {
            componenetChannelModal === 'private' &&   <input type="password" className='h-7 p-1 px-3 rounded-md w-2/3 mr-2  justify-start' />
          }
      ...
      <div className=' flex  w-full h-full '>
        <div className='flex flex-col w-1/3  text-white  bg-black h-full py-2 gap-y-2 px-1 overflow-y-hidden'>
          ...
          <div className=' px-2 h-24 flex gap-2 justify-center  items-center '>
            ...
            <Button onClick={() => setIsAddOpenChannelModal(true) } title={''} className="flex items-center justify-center text-black  w-1/3 h-3/4" > <Image alt='add groups' width={25} height={30} src={IMAgeGroups} /></Button>
          </div>
