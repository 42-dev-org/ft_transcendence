...
function HeaderSettingNotifSearchMobale(): JSX.Element {
    ...
    const closeMenus = () => setIsOpenSett(false)
    const settRef = useRef<HTMLDivElement>(null)

    useEffect(() => {
        const checkIfClickedOutside = e => {
            if (isOpenNot && notifRef?.current && !notifRef?.current?.contains(e.target)) {
                setIsOpenNot(false)
            }
        }
        ...
        const checkIfClickedOutsides = e => {
            if (IsOpenSett && settRef?.current && !settRef?.current?.contains(e.target)) {
                setIsOpenSett(false)
            }
        }
            ...
            <div className={styles["button-container"]} >
                <div className={styles['button-notif-setting']}>
                    ...
                    <div className="flex justify-center relative w-full " >
                        ...
                        {
                            isOpenNot &&
                            <div className=' dropdown-content md:w-96 w-72 z-20 flex rounded-md  h-44 overflow-hidden absolute lg:right-2 right-0  bg-[#1B1B1B] top-7' ref={notifRef}>
                                <ul className='flex flex-col w-full overflow-y-auto' >
                                    {[...Array(8)].map((_, idx) => (
                                        <li className='flex flex-row w-full h-11 justify-between items-center rounded-lg hover:bg-[#302d2d]' key={idx}>
                                            <span className=' text-white'>option</span>
                                            <Link onClick={closeMenu} href="/profile" className=' bg-[#040404] h-6 w-10 rounded-md text-white text-xs flex items-center justify-center m-1'>View</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
