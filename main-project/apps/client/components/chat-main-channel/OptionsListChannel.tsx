        ...
        <div className='flex flex-col justify-center items-center w-full'>
          {
            props.admins.map((_, i) => (
              <ListOfAdminsChannal
                ...
                uId={_.uid}
                menuList={menuList}
                setMenuList={props.role === 'owner' ? ["Remove Role Admin", 'invite for game'] : ['invite for game']}
              />
            ))
          }
        </div>
        ...
        <div className='flex flex-col justify-center items-center w-full'>
          {
            props.mut.map((_, i) => (
              <ListOfMutedChannal
                ...
                uId={_.user.uid}
                menuList={menuList}
                setMenuList={(props.role === 'admin' || props.role === 'owner') ? ["Unmuted", "invite for a game"] : ["invite for a game"]}
              />
            ))
          }
        </div>
        ...
        <div className='flex flex-col justify-center items-center w-full'>
          {
            props.ban.map((_, i) => (
              <ListOfBannedChannal
                ...
                key={_.uid}
                menuList={menuList}
                setMenuList={(props.role === 'admin' || props.role === 'owner') ? ["Unbaned"] : []}
              />
            ))
          }
        </div>
