...
function InvitationsCard({profileImage, login, uid}: {profileImage:string , login:string, uid:string}) {
    ...
    return (
        <div  className='w-full flex rounded-lg border  border-[#B2F35F] overflow-hidden h-72 flex-col '>
            ...
            <div className='bg-[#1c1e21] w-full h-full space-y-2 flex py-2 px-5 flex-col items-center '>
                ...
                <Button title='Accept invite' className='py-1 w-full' onClick={() => {sendMutation.mutate(uid)
                queryClient.invalidateQueries({ queryKey: ['send-invite'] })
                }}/>
